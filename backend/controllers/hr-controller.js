...
const importEmployeesFromCSV = async (req, res) => {
    try {
        ...
        console.log(`Parsed ${employees.length} rows from CSV`);
        ...
        for (let i = 0; i < employees.length; i++) {
            ...
            try {
                ...
                if (existingEmployee) {
                    errors.push(`Row ${i + 1}: Employee with email ${emp.email} already exists`);
                    skipped++;
                    continue;
                }
        ...
        return res.status(500).json({
            success: false,
            message: 'Error importing employees from CSV',
            error: error.message,
        });
    }
};
