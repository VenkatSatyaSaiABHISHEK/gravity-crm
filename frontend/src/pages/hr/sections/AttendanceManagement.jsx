...
import { attendanceStateColors } from '../../../data/hr-data/attendance';
...
const AttendanceManagement = ({ employees, attendance, setAttendance }) => {
  ...
  const monthStats = useMemo(() => {
    ...
    return stats;
  }, [attendance, attendanceStatusOptions]);
  ...
  const handleSaveAttendance = () => {
    ...
    // Show success message with salary impact
    const salaryImpact = calculateSalaryImpact(selectedEmployee, attendanceStatus);
    console.log(`✅ Attendance marked: ${selectedEmployee.name} - ${attendanceStatus}`);
    if (salaryImpact.hasImpact) {
      console.log(`💰 Salary Impact: ${salaryImpact.message}`);
    }
  ...
  const calculateSalaryImpact = (employee, status) => {
    const baseSalary = parseFloat(employee.salary) || 0;
    const dailySalary = baseSalary / 30; // Assuming 30 working days
    ...
    switch (status) {
      case 'Absent':
        impact = -dailySalary;
        message = `Full day deduction: ₹${Math.round(dailySalary).toLocaleString('en-IN')}`;
        break;
      case 'Half-Day':
        impact = -dailySalary * 0.5;
        message = `Half day deduction: ₹${Math.round(dailySalary * 0.5).toLocaleString('en-IN')}`;
        break;
      case 'Unpaid-Leave':
        impact = -dailySalary;
        message = `Unpaid leave deduction: ₹${Math.round(dailySalary).toLocaleString('en-IN')}`;
        break;
      case 'Present':
      case 'Paid-Leave':
        impact = 0;
